...
The source now exposes `closureExplicitVolumeSource` and `closurePressureJacobian`, both defaulting to `true` so existing cases retain their previous behavior. The A/B probes distinguish the terms: explicit PCR ON with the closure pressure Jacobian OFF stays bounded (maximum defect 1.508%, zero capped steps over 100 steps); Jacobian ON with explicit PCR OFF reproduces the step-3 jump (43.75%); an unconfigured patched case also reproduces it (43.90%), confirming the defaults preserve the prior behavior. The candidate configuration also retains HK accuracy (first-step relative error `1.12e-12`), paired mass balance, and latent-source identity. This is evidence that the closure-specific pressure Jacobian drives the instability in this calibration, not proof that it can be removed generally. Keep the candidate as case-level experimental control and V1 as NEEDS REVIEW pending pressure/temperature sweeps, condensation, mesh/time-step checks, and non-planar validation. Do not promote `closureRelax=0` or `0.1` as a fix.
...
The script accepts an output directory followed by selected probes, including `patchedExplicitOnly100`, `patchedJacobianOnly`, and `patchedUnconfigured`. Apply the full patch series and rebuild the library before running the `patched*` probes. `audit_closure_fields.py CASE --times 0 1e-14 2e-14 3e-14` summarizes p/T/rho and phase fields around the onset. Diagnostic JSON reports are stored in `../../results/2026-09-23/v1-diag-*.json`; diagnostic or non-default term settings are deliberately marked NEEDS_REVIEW because they are not yet general validation.
