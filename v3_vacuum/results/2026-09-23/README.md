...
`v1-diag-*.json` 保存闭合通路的控制对照：无 HK、关闭反馈、低松弛、增大闭合源限幅
及关闭 HK 隐式压力耦合。`closureRelax=0` 的结果仅用于诊断，报告仍标记
NEEDS_REVIEW，因为验收要求闭合反馈保持启用。

### 闭合项 A/B 更新

源码补丁 `0002-parameterize-closure-feedback-terms.patch` 为显式体积源 PCR 与闭合压力 Jacobian 增加独立开关，默认均为 `true`，不改变既有算例行为。受控 5 步对照中，PCR 开、闭合 Jacobian 关时最大缺陷从首步 1.508% 降到第 5 步 0.073%；PCR 关、Jacobian 开时第 3 步重现 43.75% 跃升；未配置开关的补丁版也重现 43.90%，核验默认值。保留 PCR、只关闭 Jacobian 的 100 步候选运行完成，最大偏差 1.508%、无闭合限幅，HK 首步相对误差 `1.12e-12`，成对质量漂移为零，潜热恒等式误差约 `3.6e-13`。

这将当前标定案例的失稳触发项定位到闭合压力 Jacobian，但单一平面蒸发配置不足以判断该项对其他状态/网格是否必要。上述 A/B 报告全部保持 `NEEDS_REVIEW`；候选开关不得作为通用生产修复。下一阶段应对候选项做压力与温度扫描、冷凝反向、网格/时间步收敛以及非平面闭合检查，并复核符号、量纲及静态 EOS 平衡。激光/粉床阶段仍未开放。
